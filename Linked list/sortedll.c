...
LNode*  AddItem(LNode  *headPtr, int  newItem)
{
	...
	if (newt->value < headPtr->value) return newt;
	else newt->next = newt->next->next;

	while(newt)
		{
		if ( NULL == newt->next
		|| newt->next->value >= newt->value )
			{
			trail->next = newt;
			return headPtr;
			}
	...
	return headPtr;//this safeguard is not necessary
}// end of AddItem
...
LNode*  RemoveItem(LNode  *headPtr, int  targetItem)
{
	...
	do  {
		if (tracker->next->value == targetItem)
			{
			temp = tracker->next;
		...
		} while (tracker->next && tracker->value < targetItem);
